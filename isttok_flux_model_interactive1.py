...
def fullModel(IsPfc, angles, Rc, isttok_mag_,time):
    ResCopper=1e-4
    Is2Bpol2=buildIs2BpolB(isttok_mag_)
    RIc = isttok_mag['RM'] + isttok_mag['Rcopper']  * np.cos(anglesIc)
    ZIc = isttok_mag['Rcopper']  * np.sin(anglesIc)
    A2,Bs2,C2,Ds2 = buildLtiModelB(RIc, ZIc,isttok_mag_, Rc)
...
if __name__ == "__main__":
    ...
    #anglesIc=[20,70,110,130,150,175,180,200,220,240,265,275,340]
    anglesH=[ 37.189 , 70, 110, 130.573, 158.021 ,175,    192.032, 205.73,  221.719,241.719, 261.562, 330.833]
    anglesP=[ 14.271 , 43.644,  75.623, 115.103, 149.427, 176.146, 343.866, 208.595, 288.755, 237.135, 271.875, 315.363]

    anglesIc = np.radians(anglesP)
    ...
    bPolTot2_P, IcPositions_P, ic_P =fullModel(IsPfc_P, anglesIc, Rc, isttok_mag_1, timeN_P)
    bPolTot2_H, IcPositions_H, ic_H =fullModel(IsPfc_H, anglesIc, Rc, isttok_mag_2, timeN_H)
    bPolTot2_V, IcPositions_V, ic_V =fullModel(IsPfc_V, anglesIc, Rc, isttok_mag_1, timeN_V)
    ...
    %matplotlib
    plt.plot(ic_H.T[0])
    plotAllPoloid2(time_H, dataArr1, dataArr2, show=True, title='Vertical Field',  ylim=25)
    ...
    for i in range(0,2):
        l="fil "+str(i+1)
        plt.plot(time_H*1000,ic_H.T[i,:],label=l)
    plt.legend()
    plt.tight_layout()

    plt.figure()
    plt.title("Seccond quadrant")
    plt.xlabel("time [ms]")
    plt.ylabel("ic [A]")
    for i in range(2,6):
        l="fil "+str(i+1)
    ...
    plt.tight_layout()
    line11,=ax1.plot(isttok_mag['Rprb'],isttok_mag['Zprb'], "+", label="Mirnovs")
    line12,=ax1.plot(IcPositions_P[0], IcPositions_P[1],"o", label="Copper fillaments")
    ...
    for i, k in zip(range(closestMirnov-1,closestMirnov+2),range(3)):
        ...
        ax2.append(ax)
        plt.tight_layout()
        l21,=ax2[k].plot(time_X*1e-3, dataArr1[i,:])
        l22,=ax2[k].plot(time_X*1e-3, dataArr2[i,:])
        line21.append(l21)
        line22.append(l22)
    ...
    while(1):
        ...
        if key == "q":
            ...
            print ("Rc:", np.diag(Rc))
            print ("directionsH:",directions_P)
        ...
        #bPolTot2_H, IcPositions_H, ic_H =fullModel(IsPfc_H, anglesIc, Rc, isttok_mag_2, timeN_H)
        bPolTot2_P, IcPositions_P, ic_P =fullModel(IsPfc_P, anglesIc, Rc, isttok_mag_2, timeN_P)
